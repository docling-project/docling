...
root=Path.cwd(); out=root/'output/acroform-keying-failure-audit'
source=(root/'output/acroform-keying-coverage-20260910/baseline_algorithm.py').read_text()
...
variants={
 'inline_pad_0.1h':source.replace('contains(label.bbox, values[i].bbox)', 'contains(BoundingBox(l=label.bbox.l-0.1*h, t=label.bbox.t-0.1*h, r=label.bbox.r+0.1*h, b=label.bbox.b+0.1*h, coord_origin=label.bbox.coord_origin), values[i].bbox)'),
 'inline_pad_0.2h':source.replace('contains(label.bbox, values[i].bbox)', 'contains(BoundingBox(l=label.bbox.l-0.2*h, t=label.bbox.t-0.2*h, r=label.bbox.r+0.2*h, b=label.bbox.b+0.2*h, coord_origin=label.bbox.coord_origin), values[i].bbox)'),
 'no_composites':source.replace('proposed = candidates_for(values, labels, h)', 'proposed = [c for c in candidates_for(values, labels, h) if c.kind != "composite_field"]'),
 'no_questions':source.replace('proposed = candidates_for(values, labels, h)', 'proposed = [c for c in candidates_for(values, labels, h) if c.kind != "choice_group"]'),
}
results={}
