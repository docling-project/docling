...
def choice_configurations(values, labels, h):
        ...
        for side in ("right", "left"):
            ...
            for i in eligible:
                ...
                for li, caption in enumerate(labels):
                    if caption.scope != question.scope or not caption.atoms.isdisjoint(question.atoms):
                        continue
                    rect = caption.bbox
                    if abs(rect.t - widget.t) > h:
                        continue
                    inlined = widget.intersection_over_self(rect) >= INLINE_WIDGET_COVERAGE
                    if inlined and sum(values[j].bbox.intersection_over_self(rect) >= INLINE_WIDGET_COVERAGE for j in eligible) != 1:
                        continue
                    if side == "right":
                        fits = rect.l >= widget.r - 0.25*h or (inlined and rect.r > widget.r + h)
                        offset = max(0, rect.l-widget.r)
                    else:
                        fits = rect.r <= widget.l + 0.25*h or (inlined and rect.l < widget.l - h)
                    ...
                    score = math.log1p(offset/h) + abs(rect.t-widget.t)/h + 0.5*caption.fragment
            ...
            for start, first in enumerate(eligible):
                ...
                beside = q.r <= first_box.l and q.t <= first_box.b and q.b >= first_box.t
                ...
                for i in eligible[start:]:
                    ...
                    if members:
                        ...
                        reset = widget.l > prev.l+h and abs(widget.t-first_box.t) <= h and widget.t < prev.t-h
                        if not (column or row or reset):
                            break
                        # A new block-level text region between option captions
                        # provides a geometric boundary. Indented notes can stay
                        # within a wrapped option block.
                    ...
                    if len(members) < 2:
                        continue
                    envelope = BoundingBox.enclosing_bbox([values[j].bbox for j in members] + [labels[j].bbox for j in option_labels])
                    ...
                    distance = (first_box.t-q.b)/h if above else (first_box.l-q.r)/max(h, first_box.l-q.l+envelope.width)
                    consistency = sum(abs(offsets[j]-offsets[first]) for j in members)/len(members)
                    features = {
                        "question_gap": math.log1p(max(0, distance)),
                        "caption_consistency": consistency,
                        "supported_repetition": -2.0*(len(members)-1)/len(members),
                        "intervening_blocks": 0.25*boundaries,
                        "prompt_fragment": 0.5*question.fragment,
                    }
                    configurations.append(Candidate(tuple(members), qi, "choice_group", features, tuple(option_labels)))
    ...
    return [c for c in configurations if not any(
        other.label == c.label
        and other.features["intervening_blocks"] == c.features["intervening_blocks"]
        and len(other.members) > len(c.members)
        and set(zip(c.members,c.option_labels)).issubset(zip(other.members,other.option_labels))
