...
- **런타임 `pip install` 도 같은 venv 로**: 런타임에 clone 된 repo 의 `requirements.txt` 를 `init.sh` 가
  `pip install` 할 때, PATH 상 `pip` 이 `/app/.venv/bin/pip`(`--seed` 가 venv 에 심어둠) 으로 해석되어
  base deps 와 **동일한 `/app/.venv`** 에 설치된다. (별도 시스템 site-packages 로 갈라지지 않음.)
