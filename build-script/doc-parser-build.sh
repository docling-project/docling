...
# BUILD_VARIANT 분기 (이슈 #199, #236 — standard/pro 두 빌드 산출물 분리)
# standard / pro 둘 중 하나는 반드시 명시되어야 한다.
...
case "${BUILD_VARIANT}" in
  ...
  "")
    ...
    echo "          BUILD_VARIANT=pro        # 위 + 유료 PDF SDK(Synap)"
    exit 1
    ;;
  *)
    echo "[ERROR] BUILD_VARIANT 는 standard 또는 pro 만 허용됩니다 (현재: '${BUILD_VARIANT}')."
    exit 1
    ;;
esac
...
#   예) gpu+pro → :1.3.6.3-gpu-pro / cpu+standard → :1.3.6.3
...
# PDF_SDK_TOKEN  : HeechanKim-Genon/pdf_sdk 전용 read 토큰 (pro 일 때만 필수)
...
if [[ "${BUILD_VARIANT}" == "pro" && -z "${PDF_SDK_TOKEN}" ]]; then
  echo "[ERROR] pro 빌드는 PDF_SDK_TOKEN 도 필요합니다 (HeechanKim-Genon/pdf_sdk 다운로드용)."
...
# 푸시 여부 플래그
# 이슈 #236 — pro 이미지는 공유 레지스트리(Genos)에 올리지 않는다. 빌드만 하고 push 스킵.
#   (Synap 유료 SDK 가 공유 레지스트리로 새어나가는 것을 막기 위함. pro 배포는 AI Search 팀 경유)
if [[ "${BUILD_VARIANT}" == "pro" ]]; then
  echo "[INFO] BUILD_VARIANT=pro — 공유 레지스트리 push 를 건너뜁니다 (이미지 빌드만 수행)."
  echo "[INFO] pro 이미지 배포가 필요하면 AI Search 팀에 문의하세요."
