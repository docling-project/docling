...
from docling.datamodel.pipeline_options_vlm_model import NuExtractVlmOptions
...
class ExtractionVlmPipeline(BaseExtractionPipeline):
    def __init__(self, pipeline_options: ExtractionPipelineOptions):
        super().__init__(pipeline_options)
        
        # Initialize VLM model with default options
        self.vlm_options = NuExtractVlmOptions(repo_id="numind/NuExtract-2.0-2B", prompt="")
        self.accelerator_options = AcceleratorOptions()
        
        # Create VLM model instance
        self.vlm_model = NuExtractTransformersModel(
            enabled=True,
            artifacts_path=None,  # Will download automatically
            accelerator_options=self.accelerator_options,
            vlm_options=self.vlm_options,
        )

    def _extract_data(self, ext_res: ExtractionResult, template: Optional[Union[str, Dict[str, Any], BaseModel]] = None) -> ExtractionResult:
            ...
            for page_num in range(page_count):
                try:
                    ...
                    if page_backend.is_valid():
                        ...
                        page_image = page_backend.get_page_image(scale=1.0)
    ...
    def _serialize_template(self, template: Union[str, Dict[str, Any], BaseModel]) -> str:
    ...
    @classmethod
    def get_default_options(cls) -> ExtractionPipelineOptions:
        return ExtractionPipelineOptions()
