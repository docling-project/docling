...
CLASS_TO_DOCITEM_LABEL: dict[str, DocItemLabel] = {
    "Caption": DocItemLabel.CAPTION,
    "Footnote": DocItemLabel.FOOTNOTE,
    "Equation-Block": DocItemLabel.FORMULA,
    "List-Group": DocItemLabel.LIST_ITEM,          # 컨테이너 성격이지만 label은 통일
...
class GenosVlmHTMLDocumentBackend(DeclarativeDocumentBackend):
    @override
    ...
    def analyze_tag(self, tag: Tag, doc: DoclingDocument) -> None:
        # div 컨텍스트 처리 (class/data-bbox를 하위 태그에 전파)
        if tag.name == "div" and (tag.get("data-bbox") is not None or tag.get("class") is not None):
            _log.info(f"Processing div context: class={tag.get('class')}, data-bbox={tag.get('data-bbox')}")
            self._push_div_context(tag)
            try:
                self.walk(tag, doc)
            finally:
                self._pop_div_context()
            return
    ...
    def handle_table(self, element: Tag, doc: DoclingDocument) -> None:
        ...
        table_data = GenosVlmHTMLDocumentBackend.parse_table_data(element)
    ...
    # ----------------------------
    # ✅ div(class/data-bbox) helpers
    # ----------------------------
    def _parse_bbox_attr(self, bbox_str: Optional[str]) -> Optional[BoundingBox]:
        _log.debug(f"Parsing bbox attribute: {bbox_str}")
        if not bbox_str:
            return None
        try:
    ...
    def _push_div_context(self, div_tag: Tag) -> None:
        # class는 보통 list로 들어옴 (bs4)
        cls = div_tag.get("class")
        class_name = cls[0] if isinstance(cls, list) and len(cls) > 0 else (cls if isinstance(cls, str) else None)
        bbox = self._parse_bbox_attr(div_tag.get("data-bbox"))
        self._div_class_stack.append(class_name)
        self._div_bbox_stack.append(bbox)

    def _pop_div_context(self) -> None:
        if self._div_class_stack:
            self._div_class_stack.pop()
        if self._div_bbox_stack:
            self._div_bbox_stack.pop()

    def _current_class(self) -> Optional[str]:
        return self._div_class_stack[-1] if self._div_class_stack else None

    def _current_bbox(self) -> BoundingBox:
        # 컨텍스트 bbox가 있으면 그걸 사용, 없으면 기존 기본값 사용
        if self._div_bbox_stack and self._div_bbox_stack[-1] is not None:
            return self._div_bbox_stack[-1]
        return BoundingBox(l=0, t=0, r=1, b=1)

    def _label_from_current_class(self, default: DocItemLabel) -> DocItemLabel:
        cls = self._current_class()
        return CLASS_TO_DOCITEM_LABEL.get(cls, default) if cls else default

    def _prov_from_current_bbox(self, char_end: int = 0) -> ProvenanceItem:
        return ProvenanceItem(page_no=1, bbox=self._current_bbox(), charspan=(0, max(0, int(char_end))))
