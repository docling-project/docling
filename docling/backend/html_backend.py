...
class HTMLDocumentBackend(DeclarativeDocumentBackend):
    ...
    def convert(self) -> DoclingDocument:
        ...
        if title:
            self.level = 1
            self.parents[self.level] = doc.add_title(
                title.get_text(separator=" ", strip=True),
    ...
    def _handle_heading(self, tag: Tag, doc: DoclingDocument) -> None:
        ...
        # the first level is the title, if it was not set with <title>
        if level == 1 and not self.has_title:
            ...
            if self.has_title:
                self.level += 1
            _log.debug(f"New heading at level {self.level - 1}")
    ...
    def _handle_list(self, tag: Tag, doc: DoclingDocument) -> None:
        ...
        for li in tag("li", recursive=False):
