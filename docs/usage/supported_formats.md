...
| Pages | Apple Pages documents (`.pages`), both container generations: Pages 5+ (2013 onwards) via its `Index/*.iwa` archives and iWork '09 via `index.xml`. Body text with titles, headings, character formatting, tables and text boxes (requires the `format-iwork` extra) |
| Markdown | |
