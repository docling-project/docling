...
| `unlimited_ocr` | Unlimited-OCR-3B | 3B | CUDA | ❌ | SGLang/OpenAI-compatible | ❌ | Markdown |
...
`unlimited_ocr` uses Baidu's custom Transformers `infer()` path for inline CUDA inference and SGLang-style OpenAI-compatible serving parameters for remote inference.
