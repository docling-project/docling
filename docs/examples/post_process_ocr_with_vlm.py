            ...
            for c in element.graph.cells:
                ...
                if good_bbox:
                    ...
                    if is_empty:
                        ...
                        print(f"Detected empty form item image crop: {rem_frac} - {debug}")
                    else:
                        result.append(
                                ...
                                if is_empty:
                                    if SHOW_EMPTY_CROPS:
                                        try:
                                            ...
                                            print(f"Error with image: {e}")
                                    print(f"Detected empty table cell image crop: {rem_frac} - {debug}")
                                else:
...
def process_pdf(pdf_path: Path, scratch_dir: Path, out_dir: Path):
    ...
    if final_json.exists() and final_json.stat().st_size > 0:
        print("Result already found here: '{}', aborting...".format(final_json))
        return  # already done
