...
`extract_all()` for many, both returning `ExtractionDocumentResult`s. The
`ExtractionTarget` is the contract (what to extract). `options`
(`ExtractDocumentsOptions`) is purely operational — model preset, decode mode,
input channel, page range — and defaults to the server's defaults.
...
def main() -> None:
    with DoclingServiceClient(
        ...
        # One document.
        try:
            document = client.extract(SINGLE, TARGET)
        except ExtractionError as exc:
            print("extract() failed:", exc)
        else:
            print("extract():", document.filename, document.status.value)
