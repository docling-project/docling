...
| `deepseecocr` | Installs the [DeepSeek-OCR](https://github.com/deepseek-ai/DeepSeek-OCR) VLM-based OCR engine. Supports CUDA (NVIDIA GPU) and MPS (Apple Silicon M1/M2/M3/M4). MPS requires PyTorch 2.7.0+. |
...
| [DeepSeek-OCR](https://github.com/deepseek-ai/DeepSeek-OCR) | `deepseecocr` extra or via `pip install transformers torch`. Supports CUDA (NVIDIA) and MPS (Apple Silicon M1/M2/M3/M4 with PyTorch 2.7.0+). | `DeepSeekOcrOptions` |
