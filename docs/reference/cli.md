...
| `--pdf-backend` | `pypdfium2`, `threaded_docling_parse` | `threaded_docling_parse` | The PDF backend to use. The deprecated `docling_parse` and `dlparse_*` values remain accepted for compatibility but are hidden from CLI help. |
