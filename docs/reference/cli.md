...
| `--ocr-lang` | `text` |  | Comma-separated list of OCR languages as BCP-47 tags, e.g. 'en,de' or 'zh-Hant'. The selected engine's own language codes are accepted too and mean what that engine means by them, so '--ocr-engine rapidocr --ocr-lang ch' is Simplified Chinese. Omit the option for the engine's default languages, or pass an empty value (--ocr-lang '') to let the engine choose, which for Tesseract is per-page script detection. 'mul' selects a multilingual model on the engines that ship one; to skip OCR entirely use --no-ocr. |
