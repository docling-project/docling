...
| `--force-ocr` / `--no-force-ocr` | flag | `false` | **Deprecated** — use `--ocr-mode full_page_ocr`. Replace any existing text with OCR generated text over the full content. |
| `--ocr-mode` | `full_page_ocr`, `pdf_bitmaps_only`, `cluster_ocr` | `pdf_bitmaps_only` | Which document regions are fed to the OCR engine. |
