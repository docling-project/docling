...
5. [파일 형식별 처리 흐름](#파일-형식별-처리-흐름)
6. [API 요청 파라미터](#api-요청-파라미터)
...
문서를 파싱하여 element 목록을 반환합니다. Parser 전처리기에서만 동작합니다.
...
> `/parser`를 지원하지 않는 전처리기(`IS_PARSER`가 없는 경우)에 요청하면 `code: 1`과 함께 "현재 설치된 전처리기는 /parser API를 지원하지 않습니다." 메시지가 반환됩니다.
...
| 파라미터 | 타입 | 기본값 | 설명 |
|----------|------|--------|------|
| `save_images` | `bool` | `true` | 파싱 중 이미지 파일 저장 여부. `false`로 설정하면 속도가 빨라짐 |
| `include_wmf` | `bool` | `false` | WMF(Windows Metafile) 이미지 포함 여부 |

> `save_images` 또는 `include_wmf` 값이 이전 호출과 다르면 내부 컨버터를 재생성합니다.
...
```bash
curl -X POST http://localhost:7084/parser \
...
```python
requests.post("http://localhost:7084/parser", json={
    "file_path": "/data/documents/contract.hwpx",
    "params": {
        "use_hwp_sdk": False
    }
})
...
response = requests.post("http://localhost:7084/parser", json={
    "file_path": "/data/documents/report.pdf",
    "params": {}
})
result = response.json()
...
```python
requests.post("http://localhost:7084/parser", json={
    "file_path": "/data/tables/sales.csv",
    "params": {}
})
# 반환 예시:
# {
#   "elements": [
#     {"category": "table", "content": "<table>...</table>", "page": 1, ...}
#   ],
#   "usage": {"pages": 1}
# }
```
