...
## 5. 공통 기능 — 민감정보 분류/마스킹 (`guardrail_masking`, #315)

청크를 생성하는 전처리기(첨부용/변환용/적재용 지능형, 그리고 청킹 API)는 문서의 민감정보(주민번호·연락처
같은 정형 PII, 부동산·인사 같은 의미 범주)를 **GenOS 분류 워크플로우** 에 위임해 판별하고, 그 결과를
청크 메타(`content_category`)에 라벨로 붙입니다(옵션으로 마스킹 치환). 파싱용은 청크가 없어 대상이 아닙니다.

- 전처리기 쪽 사용법(config·동작·출력 `content_category`)은 각 전처리기 매뉴얼의 「민감정보 분류/마스킹」 절 참고.
- 워크플로우를 만들고 배포해 전처리기와 연결하는 절차(운영용): [민감정보 분류 워크플로우 구성·배포 안내](guardrail_workflow_setup.md)
