...
각 옵션의 의미·튜닝 기준이 궁금하면 [전처리기 소개](intro.md) 와 [intelligent_processor](intelligent_processor.md) 등의 reference 문서를 참고하세요.
...
- Doc Parser가 호출할 모델 서빙이 GenOS에 등록·서빙 중. 구체적으로:
  - **Layout 모델 서빙** (예: dots.ocr) — `genos_layout` 방식(기본)일 때 호출됩니다.
  - **Enrichment LLM 서빙** — TOC 생성·메타데이터 추출·이미지 설명에 공통으로 사용.
  - **임베딩 모델 서빙** (예: bge-m3) — 벡터 DB가 호출.
  - (paddleocr은 예외. paddleocr은 본 과정을 통해 같이 설치 예정)
...
① Doc Parser 이미지 (사내 GenOS 운영계에서 빌드 이미지 사전 다운 후, 
                   해당 이미지를 사이트에서 레지스트리/DB 등록)         ③ 전처리기 생성 (facade 코드 & YAML 붙여넣기)
② PaddleOCR 이미지 (사이트에서 직접 빌드후, 레지스트리/DB 등록)          ④ YAML 수정 후 리소스 파일로 첨부
...
- 사이트 GenOS 레지스트리·DB에 등록은 [`genon/preprocessor/scripts/register_image.sh`](https://github.com/genonai/doc_parser/blob/develop/genon/preprocessor/scripts/register_image.sh) 로 처리합니다. 
  - 동일 디렉토리의 `register.config` 에 받아온 이미지와 같은 `IMAGE_VERSION` / `BUILD_VARIANT` / `HW_VARIANT` 를 입력한 뒤 실행하세요.
- 빌드 정책·variant 의미·태그 규칙 등 빌드 측 상세는 [`genon/README.md` "전처리기 빌드 및 등록"](https://github.com/genonai/doc_parser/blob/develop/genon/README.md) 참고.

### 2.2 PaddleOCR 서버 (사이트 직접 빌드)

OCR 엔진(Paddle)은 Doc Parser 본체와 달리 **사이트에서 직접 빌드·배포**합니다. 빌드 절차는 별도 문서에 정리되어 있으니, 해당 문서를 따라 진행하세요.

- PaddleOCR 빌드/배포 가이드: [`genon/README.md#paddle-ocr-빌드-및-배포`](https://github.com/genonai/doc_parser/blob/develop/genon/README.md#paddle-ocr-%EB%B9%8C%EB%93%9C-%EB%B0%8F-%EB%B0%B0%ED%8F%AC)
...
- **Layout 서빙 ID** (dots.ocr): `<LAYOUT_SERVING_ID>`
- **Enrichment LLM 서빙 ID**: `<ENRICHMENT_SERVING_ID>` — TOC / 메타데이터 / 이미지 설명 생성에서 **모두 같은 ID를 공유**합니다.
...
> [사진 위치: 좌측 메뉴 "관리 > 리소스 > 전처리기" 선택 + 우측 "전처리기 생성" 버튼 클릭 화면]

다이얼로그에 다음을 입력합니다.

| 필드 | 입력 값 |
|---|---|
| 전처리리 이름 | 사이트에서 쓸 이름 (예: `적재용 pdf`) |
| 상세 설명 | 자유 |
| 관리 그룹 | 사이트 운영 그룹 |
| 확장자 | 처리할 확장자 (예: `pdf`) |
...
> [사진 위치: "전처리기 생성" 다이얼로그 — 이름·확장자·코드·파라미터·GPU 할당 필드가 모두 보이는 화면]
...
> [사진 위치: PPT 32p의 "사내망 로컬 PC에서 사용시 예시 vs Genos에 배포시 예시" 비교 캡처 — URL/api_key 차이가 보이도록]
...
> 위 파일들은 운영 표준 묶음으로 레포의 [`genon/preprocessor/resource/`](https://github.com/genonai/doc_parser/tree/develop/genon/preprocessor/resource) 폴더에 한 세트로 들어 있습니다. **그 폴더 내용을 통째로 가져와서, yaml만 사이트에 맞게 수정한 뒤 같은 묶음을 그대로 업로드**하면 됩니다.

> [사진 위치: 전처리기 상세 화면의 "리소스 목록" 탭 + "업로드" 버튼 + 파일 업로드 다이얼로그 (PPT 35p와 동일)]
...
> 업로드 가능한 확장자에 `.yaml` / `.md` 가 포함되어 있는지 확인하세요. 다이얼로그 안내문에 표시됩니다.
...
> [사진 위치: "전처리기 배포" 다이얼로그 — 도커 이미지 드롭다운, 인스턴스 타입, 복제본 필드 보이는 화면]
...
> [사진 위치: PPT 37p — "배포 완료" 상태 화면, 그리고 옆에 "할당 대기 중" 트러블 상황 화면]

배포가 완료되면, GenOS 게이트웨이를 통해 다음 형태로 health 엔드포인트가 응답합니다(클러스터 내부에서 확인용).

```bash
curl http://<gateway-host>:<port>/preprocessor/<id>/healthcheck \
...
> [사진 위치: PPT 38p — 좌측 메뉴 "데이터 > 벡터 DB" + "벡터 DB 생성" 다이얼로그 (데이터 형식이 "문서"로 선택된 상태)]
...
> [사진 위치: PPT 39p — 문서 추가 1단계 다이얼로그 (임베딩 서빙·문서 유형·파라미터 필드 보이는 형태) + PPT 40p — 2단계 파일 업로드 + 처리 완료 후 결과 확인 화면]
...
> [사진 위치: PPT 41p — 전처리기 상세의 "컨테이너 상태 > 컨테이너 로그" 화면]
...
> [사진 위치: PPT 42p — k9s 에서 `preprocessor-<id>` deployment / pod 선택 + 로그 화면]
...
- **청킹이 적절한가?** 청크가 너무 크거나 문맥이 끊기면, 손 댈 수 있는 범위가 facade 별로 다릅니다.
  - `attachment_processor.py` — kwargs `chunker_type` 으로 `recursive`(기본) ↔ `hybrid` 전환 가능. 결과가 단조롭거나 너무 거칠면 다른 쪽으로 바꿔 비교해 보세요 (3단계 다이얼로그의 **파라미터** 필드에 `{"chunker_type":"hybrid"}` 식으로 지정).
...
Facade 코드 자체를 사이트에 맞게 수정해야 하는 케이스(메타데이터 추가, 새 확장자 처리, 청킹 알고리즘 변경 등)는 **3단계 "코드" 영역의 facade 소스를 사이트 운영자가 직접 수정**해 다시 저장하면 됩니다. 안전한 수정 범위와 패턴은 facade reference 문서들을 참고하세요.
...
- 이미지 빌드·등록 측 상세: [`genon/README.md`](https://github.com/genonai/doc_parser/blob/develop/genon/README.md)
- PaddleOCR 서버 빌드: [`genon/serving/paddle/README.md`](https://github.com/genonai/doc_parser/blob/develop/genon/serving/paddle/README.md)
- HWP→PDF 변환 chain·환경변수 override: [`genon/preprocessor/docker/README.md`](https://github.com/genonai/doc_parser/blob/develop/genon/preprocessor/docker/README.md)
