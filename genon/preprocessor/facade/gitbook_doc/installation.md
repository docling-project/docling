...
> **예외 — rhwp / LibreOffice 를 빼야 하는 사이트:** 사이트 정책상 `rhwp` · `LibreOffice` 패키지를 이미지에 넣을 수 없는 경우(예: 한국은행)에는, **운영계에 올라온 이미지를 그대로 쓸 수 없고 전처리기 이미지를 새로 빌드해야 합니다** (운영 이미지에는 두 패키지가 포함돼 있음). 빌드 시 `INSTALL_LIBREOFFICE=false` / `INSTALL_RHWP=false` 로 끄고 빌드하며, 절차는 [`genon/README.md` "A-2. (선택) rhwp / LibreOffice 제외 빌드"](../../../README.md#a-2-선택-rhwp--libreoffice-제외-빌드-이슈-286) 를 참고하세요. <br> 이렇게 빌드한 이미지는 HWP/오피스 문서를 PDF 로 변환하지 못하므로(변환 backend 미포함), 지능형 전처리기 사용시, **해당 사이트에서는 PDF로 변환된 문서로 직접 업로드**해야 합니다.
