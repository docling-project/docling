...
파싱 결과(docling JSON)를 입력받아 청킹만 수행하고 청크 목록(`GenOSVectorMeta`)을 반환하는 별도 전처리기(`chunking_processor`)가 처리합니다. 해당 전처리기는 `IS_CHUNKER` 마커로 식별되며, 요청 본문은 다음과 같습니다.
...
E2E 흐름: `POST /parser (output.format=docling)` → `data.document` → `POST /chunker (params.document=…)` → 청크(`GenOSVectorMeta`) 목록.
