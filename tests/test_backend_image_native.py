...
def _make_png_stream(width: int = 64, height: int = 48, color=(123, 45, 67)) -> DocumentStream:
...
def _make_multipage_tiff_stream(num_pages: int = 3, size=(32, 32)) -> DocumentStream:
    frames = [Image.new("RGB", size, (i * 10 % 255, i * 20 % 255, i * 30 % 255)) for i in range(num_pages)]
    buf = BytesIO()
