...
def test_num_fmt_enum_markers(tmp_path):
    """List markers must follow w:numFmt, not a raw decimal counter.

    Word stores the displayed form in numbering.xml (``w:numFmt`` + ``w:lvlText``).
    ``_build_enum_marker`` used to substitute ``str(counter)`` for every ``%N``
    placeholder, so ``lowerLetter``/``upperLetter``/``lowerRoman``/``upperRoman``
    /``decimalZero`` all rendered as ``1.``/``2.`` instead of ``(a)``/``b)``/``A.``
    /``i.``/``II.``/``01.``.
    """
    ...
    specs = (
        ("800", "900", "lowerLetter", "(%1)"),
        ("801", "901", "lowerLetter", "%1)"),
        ("802", "902", "upperLetter", "%1."),
        ("803", "903", "lowerRoman", "%1."),
        ("804", "904", "upperRoman", "%1."),
        ("805", "905", "decimalZero", "%1."),
    )
    for abstract_id, num_id, num_fmt, lvl_text_val in specs:
        abstract_num = OxmlElement("w:abstractNum")
    ...
    backend.list_counters[(900, 0)] = 1
    assert backend._build_enum_marker(900, 0) == "(a)"

    backend.list_counters[(901, 0)] = 2
    assert backend._build_enum_marker(901, 0) == "b)"

    backend.list_counters[(902, 0)] = 1
    assert backend._build_enum_marker(902, 0) == "A."

    backend.list_counters[(903, 0)] = 1
    assert backend._build_enum_marker(903, 0) == "i."

    backend.list_counters[(904, 0)] = 2
    assert backend._build_enum_marker(904, 0) == "II."

    backend.list_counters[(905, 0)] = 1
    assert backend._build_enum_marker(905, 0) == "01."
