...
class _UnusableCropBackend:
    """A backend whose crop is unusable, the way a failed render is."""
...
@pytest.mark.parametrize(
    "image",
    [None, Image.new("RGB", (0, 8), "white")],
    ids=["no_image", "zero_width"],
)
def test_cell_with_an_unusable_crop_is_skipped(image: Image.Image | None) -> None:
    """A crop the backend could not render must not reach the engine as-is."""
    doc = _doc_with_tables([_cell("a^2 + 8 = 12", 0, 0)])
    page = Page(page_no=1, size=PAGE_SIZE)
    page._backend = _UnusableCropBackend(image)

    assert (
        _make_stage().prepare_element(
            conv_res=_conv_res(doc, pages=[page]), element=doc.tables[0]
        )
        is None
    )


@pytest.mark.parametrize("case", ["disabled", "engine_gone"])
def test_call_passes_tables_through_when_the_stage_cannot_run(case: str) -> None:
    """``__call__`` re-checks the gate, because it is reachable independently."""
    engine = _DummyEngine()
    stage = _make_stage(engine=engine)
    doc = _doc_with_tables([_cell("a^2 + 8 = 12", 0, 0)])
    table = doc.tables[0]
    element = stage.prepare_element(conv_res=_conv_res(doc), element=table)
    assert element is not None

    if case == "disabled":
        stage.enabled = False
    else:
        stage._code_formula_model.engine = None

    assert list(stage(doc=doc, element_batch=[element])) == [table]
    assert _rich_cells(table) == []
    assert doc.groups == []
    assert engine.received_inputs == []


def test_cell_that_became_rich_mid_flight_is_not_overwritten() -> None:
    """Preparation and writing are separated by the engine call, so the cell can change."""
    doc = _doc_with_tables([_cell("a^2 + 8 = 12", 0, 0)])
    table = doc.tables[0]
    stage = _make_stage()
    element = stage.prepare_element(conv_res=_conv_res(doc), element=table)
    ...
    # Something else claims the cell in between -- what the reading-order stage does for a
    # picture cell.
    group = doc.add_group(label=GroupLabel.UNSPECIFIED, name="claimed", parent=table)
    doc.add_text(label=DocItemLabel.TEXT, text="claimed", parent=group)
    plain = table.data.table_cells[0]
    table.data.table_cells[0] = RichTableCell(
        **plain.model_dump(exclude={"ref"}), ref=group.get_ref()
    )

    list(stage(doc=doc, element_batch=[element]))

    assert table.data.table_cells[0].ref.cref == group.self_ref
    assert [g.name for g in doc.groups] == ["claimed"]


def test_one_failing_cell_does_not_lose_the_rest_of_the_table(
    caplog, monkeypatch
) -> None:
    doc = _doc_with_tables([_cell("a^2 + 8 = 12", 0, 0), _cell("b^2 = 4", 0, 1)])
    stage = _make_stage()
    write_rich_cell = stage._write_rich_cell

    def flaky(*, doc, table, cell_index, texts):
        if cell_index == 0:
            raise RuntimeError("cell exploded")
        return write_rich_cell(doc=doc, table=table, cell_index=cell_index, texts=texts)

    monkeypatch.setattr(stage, "_write_rich_cell", flaky)

    with caplog.at_level("WARNING"):
        _enrich(stage, doc, _conv_res(doc))

    assert "Failed to attach recognised formula" in caplog.text
    rich = _rich_cells(doc.tables[0])
    assert len(rich) == 1
    assert rich[0].start_col_offset_idx == 1


def test_cell_bbox_is_converted_to_a_top_left_table_prov() -> None:
    """The mirror of the BOTTOMLEFT case: derived in both directions, not assumed."""
    doc = _doc_with_tables([_cell("a^2 + 8 = 12", 0, 0)])
    table = doc.tables[0]
    # The cell keeps its BOTTOMLEFT bbox (t=700, b=650); the table's provenance does not.
    table.prov[0].bbox = BoundingBox(
        l=40.0, r=560.0, t=72.0, b=692.0, coord_origin=CoordOrigin.TOPLEFT
    )
    _enrich(_make_stage(), doc, _conv_res(doc))

    formula = _rich_cells(table)[0].ref.resolve(doc).children[0].resolve(doc)
    prov_bbox = formula.prov[0].bbox
    assert prov_bbox.coord_origin is CoordOrigin.TOPLEFT
    # 792 - 700 = 92, 792 - 650 = 142
    assert (prov_bbox.t, prov_bbox.b) == (92.0, 142.0)


@pytest.mark.parametrize(
    "case", ["no_prov", "no_bbox", "page_missing", "page_size_missing"]
)
def test_cell_prov_bbox_declines_without_page_geometry(case: str, monkeypatch) -> None:
    """No provenance beats provenance in an origin that cannot be resolved."""
    doc = _doc_with_tables([_cell("a^2 + 8 = 12", 0, 0)])
    table = doc.tables[0]
    ...
    if case == "no_prov":
        ...
        cell.bbox = None
    elif case == "page_missing":
        doc.pages.clear()
    else:
        # Defensive: PageItem.size is required, so this shape is not declared.
        monkeypatch.setattr(doc.pages[1], "size", None)

    assert (
        TableCellFormulaVlmModel._cell_prov_bbox(doc=doc, table=table, cell=cell)
        is None
    )
