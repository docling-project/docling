...
def test_list_after_numbered_heading_stays_in_its_section(tmp_path):
    """A list following a numbered heading must stay inside that heading.

    Heading styles that carry ``w:numPr`` (numbered outline headings) are
    reported as list items by ``_get_numId_and_ilvl``, and a heading does not
    close an open list. ``_add_heading`` clears the tail of ``parents`` but used
    to leave ``level_at_new_list`` untouched, so the first list item after the
    heading derived ``use_level`` from the stale value, found ``None`` in the
    corresponding ``parents`` slot and had its group attached to the body root.
    Those items were then serialized after everything else in the document.
    """
    ...
    def set_num_pr(element, num_id: str):
        num_pr = OxmlElement("w:numPr")
        ilvl = OxmlElement("w:ilvl")
        ilvl.set(qn("w:val"), "0")
        num_pr.append(ilvl)
        ...
        num_id_elem.set(qn("w:val"), num_id)
        num_pr.append(num_id_elem)
        element.get_or_add_pPr().insert(0, num_pr)

    def add_item(text: str):
        paragraph = doc.add_paragraph(text, style="List Paragraph")
        set_num_pr(paragraph._element, "901")

    add_numbering("900", "901")  # body list
    add_numbering("910", "911")  # heading outline numbering
    # Numbered outline headings: the numbering sits on the style, not on the
    # paragraph, which is what corporate templates produce.
    set_num_pr(doc.styles["Heading 1"].element, "911")

    doc.add_paragraph("First section", style="Heading 1")
    doc.add_paragraph("First subsection", style="Heading 2")
    add_item("first subsection item")

    doc.add_paragraph("Second section", style="Heading 1")
    add_item("second section item")

    doc.add_paragraph("Second subsection", style="Heading 2")
    add_item("second subsection item")

    docx_path = tmp_path / "list_after_numbered_heading.docx"
    doc.save(str(docx_path))
    ...
    texts = [
        item.text
        for item, _ in converted.iterate_items()
        if isinstance(item, (ListItem, SectionHeaderItem))
    ]

    assert texts == [
        "1 First section",
        "First subsection",
        "first subsection item",
        "2 Second section",
        "second section item",
        "Second subsection",
        "second subsection item",
    ]
