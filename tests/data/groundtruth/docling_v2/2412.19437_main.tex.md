...
$W^{KR} \in \mathbb{R}^{d\_h^R \times d}$ is the matrix used to produce the decoupled key that carries Rotary Positional Embedding (RoPE)
...
Note that for MLA, only the blue-boxed vectors (i.e., $\color{blue} \mathbf{c}\_{t}^{KV}$ and $\color{blue}\mathbf{k}\_{t}^{R}$) need to be cached during generation, which results in significantly reduced KV cache while maintaining performance comparable to standard Multi-Head Attention (MHA)
...
and $W^{QR} \in \mathbb{R}^{d\_h^R n\_h \times d\_c^{\prime}}$
...
where $W^{O} \in \mathbb{R}^{d \times d\_h n\_h}$
...
In short, we ensure that each token will be sent to at most $M$ nodes, which are selected according to the sum of the highest $\frac{K\_r}{M}$
...
, DualPipe significantly reduces the pipeline bubbles while only increasing the peak activation memory by $\frac{1}{PP}$ times.
...
selects only 8 routed experts in practice, it can scale up this number to a maximum of 13 experts (4 nodes $\times$
...
In the current implementation, when the $N\_C$
...
with hyper-parameters set to $\beta\_1=0.9$, $\beta\_2=0.95$, and $\mathrm{weight\_decay}=0.1$. 
...
The learning rate for both phases is set to $7.3 \times 10^{-6}$
