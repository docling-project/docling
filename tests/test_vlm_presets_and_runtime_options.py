...
class TestApiEngineParamOverride:
    ...
    These tests exercise the real ApiVlmEngine.predict_batch code path with
    mocked API calls to verify param construction.
    """

    def _make_engine(self, user_params: dict) -> "ApiVlmEngine":
        """Create ApiVlmEngine with given user params."""
    ...
    def _run_engine_and_capture_params(self, engine, **input_kwargs):
        """Run predict_batch and capture the api_params passed to the API."""
        ...
        with patch(
            ...
            mock_api.return_value = ("result", 10, "stop")
            engine.predict_batch([input_data])
    ...
    def test_user_overrides_max_tokens(self):
        ...
        engine = self._make_engine({"max_tokens": 8192})
        params = self._run_engine_and_capture_params(engine)
