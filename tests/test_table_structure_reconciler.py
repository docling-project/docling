...
def test_reassign_text_preserving_rows_splits_horizontal_text_only():
    column_grid = ColumnGridCandidate(
    ...
    model_cells = [
        fake_model_cell("Header", 0, 1, 0, 3, 0, 300),
        fake_model_cell("COL A COL B", 1, 2, 1, 3, 100, 300),
    ]
    intervals = [
        interval(120, 150, 45, "COL A"),
        interval(220, 260, 45, "COL B"),
    ]

    reassigned = reassign_text_preserving_rows(
        model_cells,
        intervals,
    ...
    assert reassigned.num_rows == 2
    assert reassigned.num_cols == 3

    by_text = {cell.text: cell for cell in reassigned.table_cells}

    assert by_text["COL A"].start_row_offset_idx == 1
    assert by_text["COL A"].end_row_offset_idx == 2
    assert by_text["COL A"].start_col_offset_idx == 1
    assert by_text["COL A"].end_col_offset_idx == 2

    assert by_text["COL B"].start_row_offset_idx == 1
    assert by_text["COL B"].end_row_offset_idx == 2
    assert by_text["COL B"].start_col_offset_idx == 2
    assert by_text["COL B"].end_col_offset_idx == 3


def test_reassign_text_preserving_rows_splits_multiple_rows_without_changing_rows():
    column_boundaries = (0.0, 100.0, 200.0, 300.0, 400.0, 500.0)

    intervals = [
        interval(10, 40, 50, "ROW LABEL"),
        interval(10, 40, 150, "GROUP A"),
        interval(120, 150, 150, "COL A"),
        interval(220, 260, 150, "COL B"),
        interval(330, 370, 150, "COL C"),
        interval(430, 470, 150, "COL D"),
        interval(10, 40, 250, "GROUP B"),
        interval(120, 150, 250, "VAL A"),
        interval(220, 280, 250, "VAL B"),
        interval(330, 370, 250, "ITEM A"),
        interval(430, 470, 250, "STATE A"),
        interval(10, 40, 350, "GROUP C"),
        interval(120, 150, 350, "VAL C"),
        interval(220, 280, 350, "VAL D"),
        interval(330, 370, 350, "ITEM B"),
        interval(430, 470, 350, "STATE B"),
    ]

    model_cells = [
        fake_model_cell("ROW LABEL", 0, 1, 0, 1, 0, 100),
        fake_model_cell("GROUP", 1, 4, 0, 1, 0, 100),
        fake_model_cell("COL A COL B", 1, 2, 1, 3, 100, 300),
        fake_model_cell("COL C", 1, 2, 3, 4, 300, 400),
        fake_model_cell("COL D", 1, 2, 4, 5, 400, 500),
        fake_model_cell("VAL A VAL B", 2, 3, 1, 3, 100, 300),
        fake_model_cell("ITEM A", 2, 3, 3, 4, 300, 400),
        fake_model_cell("STATE A", 2, 3, 4, 5, 400, 500),
        fake_model_cell("VAL C VAL D", 3, 4, 1, 3, 100, 300),
        fake_model_cell("ITEM B", 3, 4, 3, 4, 300, 400),
        fake_model_cell("STATE B", 3, 4, 4, 5, 400, 500),
    ]
    ...
    for cell in model_cells:
        cell.bbox = FakeBBox(
            cell.bbox.l,
            cell.bbox.r,
            cell.start_row_offset_idx * 100.0,
            cell.end_row_offset_idx * 100.0,
        )
    ...
    assert reassigned is not None
    assert reassigned.num_rows == 4
    assert reassigned.num_cols == 5
    ...
    assert by_text["VAL A"].start_row_offset_idx == 2
    assert by_text["VAL A"].end_row_offset_idx == 3
    assert by_text["VAL B"].start_row_offset_idx == 2
    assert by_text["VAL B"].end_row_offset_idx == 3

    assert by_text["VAL C"].start_row_offset_idx == 3
    assert by_text["VAL C"].end_row_offset_idx == 4
    assert by_text["VAL D"].start_row_offset_idx == 3
    assert by_text["VAL D"].end_row_offset_idx == 4

    group_cells = [
        cell
        for cell in reassigned.table_cells
...
def test_reassign_text_preserving_rows_does_not_populate_empty_cells():
    model_cells = [
        fake_model_cell("", 0, 1, 1, 3, 100, 300),
    ]
    intervals = [
        interval(120, 150, 45, "A"),
        interval(220, 260, 45, "B"),
    ]
    ...
    assert reassigned is not None
    assert len(reassigned.table_cells) == 1
    assert reassigned.table_cells[0].text == ""
    assert reassigned.table_cells[0].start_col_offset_idx == 1
    assert reassigned.table_cells[0].end_col_offset_idx == 3


def test_reassign_text_preserving_rows_ignores_text_outside_cell_vertical_bbox():
    model_cells = [
        fake_model_cell("A B", 0, 1, 1, 3, 100, 300),
    ]
    model_cells[0].bbox = FakeBBox(100, 300, 100, 120)

    intervals = [
        interval(120, 150, 80, "A"),
        interval(220, 260, 150, "B"),
    ]
    ...
    assert reassigned is not None
    assert len(reassigned.table_cells) == 1
    assert reassigned.table_cells[0].text == "A B"
    assert reassigned.table_cells[0].start_col_offset_idx == 1
    assert reassigned.table_cells[0].end_col_offset_idx == 3
